...
def test_clebsch_gordan_tensor_realizations_shape_and_cache_contract() -> None:
    theory = SO3RepresentationTheory()
    core = TensorTrainCore(1, 1, 0, 1)
    integer_tensor = theory.clebsch_gordan_tensor(core, 7)
    tensor = theory.clebsch_gordan_tensor(core, 0)

    assert integer_tensor.shape == (3, 3, 1)
    assert integer_tensor.dtype == np.uint64
    assert integer_tensor.flags.writeable is False
    assert theory.clebsch_gordan_tensor(core, 7) is integer_tensor
    ...
    expected_complex_tensor = np.asarray(
        [[[0], [0], [4]], [[0], [-2], [0]], [[4], [0], [0]]],
        dtype=np.complex128,
    )
    assert np.array_equal(integer_tensor, expected_modular_tensor)
    assert np.array_equal(tensor, expected_complex_tensor)
    expected_modular_tensor_11 = np.asarray(
        [[[0], [0], [4]], [[0], [9], [0]], [[4], [0], [0]]],
        dtype=np.uint64,
    )
    assert np.array_equal(
        theory.clebsch_gordan_tensor(core, 11),
        expected_modular_tensor_11,
    )
...
def test_evaluate_tensor_train_uses_requested_clebsch_gordan_realization(
    ...
    class Theory:
        def clebsch_gordan_tensor(
            ...
            if modulus == 0:
                return np.asarray([[[2.5]]], dtype=np.complex128)
            return np.asarray([[[modulus - 1]]], dtype=np.uint64)
...
def test_monomial_waring_data_uses_finite_difference_rows() -> None:
    ...
    assert np.array_equal(grid[:2], np.asarray([[0, 0], [0, 1]], dtype=np.uint64))
...
def test_young_symmetrized_tensor_tree_uses_complex_waring_grid_for_modulus_zero() -> None:
    ...
    expected = np.asarray([[2 * 2 * 3]], dtype=np.complex128)
...
def test_multigraded_hilbert_series_totals_match_ordinary_series() -> None:
    multigraded = hilbert_series_so3_multigraded((1, 2), (1, 1), 1, (3, 3))
    totals = tuple(
        sum(
            dimension
            for multidegree, dimension in multigraded.items()
            if sum(multidegree) == degree
        )
        for degree in range(4)
    )

    assert totals == hilbert_series_so3((1, 2), (1, 1), 1, 3)
