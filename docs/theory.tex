...
\begin{abstract}
  ...
  In particular, we provide end-to-end implementations for the groups $\SO(2)$, $\SO(3)$, $O(2)$, and $O(3)$. We also discuss how to enforce translation and permutation equivariance within our framework.
...
Appendix~\ref{sec:appendix-copy-permutation-equivariance} explains how to enforce copy permutation equivariance.
...
\section{Copy permutation equivariance}\label{sec:appendix-copy-permutation-equivariance}
...
\]
This action commutes with $G$, since $G$ acts only on the $H_\lambda$-factors.
