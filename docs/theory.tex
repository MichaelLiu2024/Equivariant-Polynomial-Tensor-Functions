...
\title{Equivariant Learning via Minimal Integrity Bases for Invariant and Equivariant Polynomials}
...
\begin{abstract}
  ...
  Our approach enforces equivariance by a one-time computation of minimal integrity bases for the invariant algebra and covariant module of invariant and equivariant polynomials.
  ...
  To compute minimal integrity bases, we first enumerate vector space bases for the invariant algebra and covariant module as candidate generators.
  ...
  In particular, we provide end-to-end implementations for the groups $\SO(2)$, $\SO(3)$, and $S_d$, and we sketch representative applications.
...
Suppose that we wish to learn an unknown $G$-equivariant polynomial $q\colon X\to Y$.
Let $\set{m_1,\dots,m_M}\subseteq \scM_G(X,Y)$ be minimal $\scR_G(X)$-module generators, and let $\set{r_1,\dots,r_R}\subseteq \scR_G(X)$ be minimal $\dsC$-algebra generators.
Then $q$ admits a decomposition
\begin{equation}\label{eq:polynomial-ansatz}
  q(x)=\sum_{i=1}^M p_i(r_1(x),\dots,r_R(x))m_i(x),
\end{equation}
where $p\colon \dsC^R\to\dsC^M$ is some polynomial.
Thus, learning the unknown polynomial $q$ reduces to learning $p$.

Now, suppose that we wish to learn an unknown $G$-equivariant function $F\colon X\to Y$.
Replacing the polynomial $p$ with a neural network $\scN_\theta\colon \dsC^R\to\dsC^M$, we obtain a large class of $G$-equivariant functions
\begin{equation}\label{eq:neural-ansatz}
  F_\theta(x)=\sum_{i=1}^M
  \scN_{\theta,i}(r_1(x),\dots,r_R(x))m_i(x)
\end{equation}
from $X$ to $Y$, and it remains only to learn $\theta$ so that $F_\theta \approx F$. Special cases of the ansatz in Eq.~\eqref{eq:neural-ansatz} have appeared in the literature before \cite{BlumSmithVillar2023}.

Note that equivariance is built into Eq.~\eqref{eq:neural-ansatz}: the coefficients $\scN_{\theta,i}(r_1(x),\dots,r_R(x))$ depend only on invariant quantities, and the polynomials $m_i$ are equivariant by construction.
Thus, the equivariance constraint is frontloaded into the one-time computation of generators, and the learning procedure need not worry about further equivariance constraints.
In particular, the neural network $\scN_\theta$ may have any standard architecture, such as a feed-forward network, residual network, attention-based scalar network, or other differentiable model, and one may use standard pointwise nonlinearities and scalar-network design choices without imposing equivariance constraints on the hidden layers of $\scN_\theta$. Moreover, the hypothesis class in Eq.~\eqref{eq:neural-ansatz} is a universal approximator for the continuous $G$-equivariant functions.
...
$\{r_1,\ldots,r_R\}\subseteq \scR_G(X)$\\[0.2em]
$\{m_1,\ldots,m_M\}\subseteq \scM_G(X,Y)$};
...
\caption{Workflow for learning $G$-equivariant functions $F\!:\!X\to Y$.
...
For the resulting triple $(G,X,Y)$, one chooses a degree bound $D_{\max}$ and performs a one-time computation of invariant generators $\{r_1,\ldots,r_R\}\subseteq \scR_G(X)$ and covariant generators $\{m_1,\ldots,m_M\}\subseteq \scM_G(X,Y)$ up to degree $D_{\max}$.
For any $G$-equivariant target function $F\!:\!X\to Y$, one trains the neural-network coefficient function $\scN_\theta\colon \dsR^R\to\dsR^M$ in the ansatz of Eq.~\eqref{eq:neural-ansatz}.
...
Section~\ref{sec:theory-and-algorithms} contains the theory and algorithms.
Section~\ref{sec:examples} works through representative examples in more detail.
Section~\ref{sec:conclusion} summarizes our work and suggests directions for future work.
Appendix~\ref{sec:appendix-proofs} provides proofs of our results.
Appendix~\ref{sec:appendix-implementation} describes implementation details, including shortcuts for the case $G = \SO(3)$.
...
We derive an algorithm to compute, up to a prescribed degree, minimal algebra generators $r_1,\dots,r_R$ for the invariant polynomials on $X$ and minimal module generators $m_1,\dots,m_M$ for the equivariant polynomials from $X$ to $Y$.
...
The main technical result underlying our framework is an explicit isotypic decomposition of the symmetric power $\scS^D(X)$ of the input representation $X$.
To see why, suppose that we are given explicit isotypic decompositions
\[
...
Thus, an explicit isotypic decomposition of $\scS^D(X)$ ultimately yields a vector space basis for $\scM_G^D(X,Y)$.

Now, suppose that $\scM_G(X,Y)$ is a finitely generated $\scR_G(X)$-module and $\scR_G(X)$ is a finitely generated $\dsC$-algebra.
Then the vector space bases produced by our framework can be used as candidate generators in a standard procedure based on the graded Nakayama lemma to extract minimal homogeneous generators for $\scM_G(X,Y)$ as an $\scR_G(X)$-module and $\scR_G(X)$ as a $\dsC$-algebra up to a given degree.
In particular, we show that when computing minimal module generators for $\scM_G(X,Y)$, it is more efficient to first use the graded Nakayama lemma to obtain minimal module generators for $\scM_G(X,H_\nu)$ and then use Eq.~\eqref{eq:output-reduction} to lift these generators to $\scM_G(X,Y)$.

In principle, the theory in this work applies to any linearly reductive group.
However, in order to implement concrete algorithms, one must provide a model for the irreps of $G$ along with the following.
...
\begin{table}[htbp]
  ...
  To move from the top--left corner to the bottom--right corner, it is algorithmically more efficient to first move down and then move right.}
...
\begin{theorem}[Universal approximation by invariant-coordinate networks]\label{thm:invariant-coordinate-universal-approximation}
  ...
  Let
  \[
    \scR_{G,\dsR}(X)\coloneq \dsR[X]^G,
    \qquad
    \scM_{G,\dsR}(X,Y)
    \coloneq
    \set{p\colon X\to Y : p\text{ is polynomial and }G\text{-equivariant}}.
  \]
  Suppose that $r_1,\dots,r_R\in \scR_{G,\dsR}(X)$ generate $\scR_{G,\dsR}(X)$ as a $\dsR$-algebra, and that $m_1,\dots,m_M\in \scM_{G,\dsR}(X,Y)$ generate $\scM_{G,\dsR}(X,Y)$ as an $\scR_{G,\dsR}(X)$-module.
  Let $\scN\subseteq C(\dsR^R,\dsR^M)$ be a neural-network class with the following density property: for every compact subset $L\subseteq \dsR^R$, the restrictions $\set{N|_L:N\in\scN}$ are dense in $C(L,\dsR^M)$ with respect to the uniform norm.

  Then the class $\scA$ of functions $F_\theta$ given by the ansatz in Eq.~\eqref{eq:neural-ansatz}, with $\scN_\theta\in\scN$, is dense in $C_G(X,Y)$ in the compact-open topology.
  Equivalently, for every compact $K\subseteq X$, every $F\in C_G(X,Y)$, and every $\varepsilon>0$, there exists $F_\theta\in\scA$ such that
  \[
    \sup_{x\in K}\left|F(x)-F_\theta(x)\right|<\varepsilon.
  \]
  The density hypothesis on $\scN$ holds, for example, when $\scN$ contains all vector-valued one-hidden-layer feed-forward networks with biases and a continuous non-polynomial activation function; this follows coordinatewise from \cite[Theorem~3.1, p.~153]{Pinkus1999}.
  More generally, for locally bounded piecewise continuous activations, the corresponding non-polynomial a.e. criterion is given in \cite[Theorem~1, p.~863]{LeshnoLinPinkusSchocken1993}.
\end{theorem}

\begin{proof}
  It is enough to prove uniform approximation on an arbitrary compact subset $K\subseteq X$.
  Fix $F\in C_G(X,Y)$ and $\varepsilon>0$.
  Choose a $G$-invariant norm on $Y$ by averaging an inner product over normalized Haar measure on $G$.
  Since $G$ is compact, the saturation
  \[
    K_G\coloneq GK
  \]
  is compact.

  By the real Stone--Weierstrass theorem, applied coordinatewise after choosing a basis of $Y$, polynomial maps $X\to Y$ are uniformly dense in $C(K_G,Y)$.
  Hence there is a polynomial map $q\colon X\to Y$ such that
  \[
  ...
  \]
  Since the action is linear and $q$ is polynomial, $\overline q$ is again a polynomial map.
  It is also $G$-equivariant: for $h\in G$, the substitution $k=gh$ gives
  \[
    \overline q(hx)
    =
  ...
  By the polynomial ansatz in Eq.~\eqref{eq:polynomial-ansatz}, applied over $\dsR$, there is a polynomial coefficient map $P=(P_1,\dots,P_M)\colon\dsR^R\to\dsR^M$ representing $\overline q$.
  Write $r=(r_1,\dots,r_R)$ and $P=(P_1,\dots,P_M)$.
  The set $r(K)\subseteq\dsR^R$ is compact, so by the density assumption on $\scN$, there exists $\scN_\theta\in\scN$ such that
  \[
    \sup_{z\in r(K)}\left|\scN_\theta(z)-P(z)\right|_2<\delta,
  \]
  where $\delta>0$ will be chosen below.
  Define $F_\theta$ by the ansatz in Eq.~\eqref{eq:neural-ansatz} using this $\scN_\theta$.
  ...
  \]
  is finite.
  Therefore, using Eqs.~\eqref{eq:polynomial-ansatz} and~\eqref{eq:neural-ansatz}, for $x\in K$,
  \[
    \begin{aligned}
...
In this section, we describe how to obtain a vector space basis for $\scM_G^D(X,H_\nu)$.
Our core theoretical result is an explicit isotypic decomposition of $\scS^D(X)$.
...
\begin{lemma}[Semistandard Young tableaux basis for Schur functors]\label{lem:ssyt-basis}
  Let $V$ be a vector space with basis $(v_1,\dots,v_n)$, let $d \in \dsN_0$, and let $\pi\vdash d$ be a partition with at most $n$ parts.
...
\begin{corollary}[Theorem~\ref{thm:isotypic-decomposition} for $\operatorname{O}(2)$]\label{cor:o2-isotypic-decomposition}
  ...
  \]
  be the $\operatorname{O}(2)$-isotypic decomposition of a finite-dimensional representation.
...
Here $\ell$ is the usual $\SO(3)$ spin and $\sigma$ is the scalar by which $-I$ acts.
Its dimension is
\[
...
Let $\pi\vdash d$ and $\lambda=(\ell,\sigma)$.
Then
\[
  e_\pi(H_\lambda^{\otimes d})
  \cong
...
Let $\widetilde G = \operatorname{Spin}^+(3,1)$ and let $G = \SO^+(3,1)$.
...
\]
The irrep label set for $\widetilde G$ is $\Lambda_{\widetilde G}=\dsN_0^2$.
For $\lambda=(a,b)\in\Lambda_{\widetilde G}$, let
\[
...
Let $\pi\vdash r$ and $\lambda=(a,b)$.
The weights of $H_\lambda$ are
\[
  (a-2i,\ b-2j),
  \qquad
...
\[
  ...
  \chi_\lambda(g^{\rho_i}),
\]
where $\chi_{\pi}^{\mathfrak S_d}$ is the symmetric-group character indexed by $\pi$, and $z_\rho$ is the centralizer order of a permutation of cycle type $\rho$.
Hence
\[
  m_{\pi,\lambda}^{\nu}
  =
...
The $\SO(3)$ backend uses the representation-theoretic conventions and formulas from Section~\ref{sec:examples-so3}.
The remaining details here are implementation shortcuts rather than part of the abstract contract.

For tensor-product constituents, the backend applies \eqref{eq:so3-Clebsch--Gordan}.
For tensor-train evaluation, it uses the Clebsch--Gordan tensors fixed in \eqref{eq:so3-wigner-3j}, evaluating an equivalent integer formula for the coefficients.
The finite-field evaluator then reduces those integer tensor entries modulo the active prime.

\begin{example}[$\SO(3)$ small-degree shortcuts]\label{ex:so3-small-degree-shortcuts}
  For small exterior and symmetric powers, the $\SO(3)$ backend may bypass the general rank-selection procedure and use explicit formulas.
  These shortcuts are not part of the general contract; they only improve efficiency in low degrees.
  ...
  \]
  These decompositions follow by applying \eqref{eq:so3-schur-multiplicity} to the partitions $(1,1)$, $(1,1,1)$, $(2)$, and $(3)$.
  The only listed multiplicity greater than $1$ is the term $2H_3$ in $\scS^3(H_3)$.
\end{example}
